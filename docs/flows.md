...
Template อีเมลเก็บที่ `docs/flow-emails/` (สำเนาอ้างอิง — ตัวจริง paste ใน JotForm)

## มาตรฐานอีเมล (ใช้กับทุก template)

**ชุดสี — เขียว/เหลือง/แดง ต้องเป็นโค้ดเดียวกันทุกที่ (ยืนยัน 2026-08-27):**

| สี | โค้ด | ใช้กับสถานะ |
|---|---|---|
| 🟢 เขียว | `#4CAF50` | อนุมัติ / สำเร็จ |
| 🟡 เหลือง | `#F5A623` | รอดำเนินการ / รับเรื่องแล้ว (pending) |
| 🔴 แดง | `#F44336` | ไม่อนุมัติ / ต้องดำเนินการ (reject / action) |
| 🟠 ส้ม (แบรนด์) | `#FA4616` | header gradient เท่านั้น — ไม่ใช่ semantic |

- badge/แถบสถานะ + ปุ่ม + แถบ accent 4px = ใช้โค้ด semantic ตรง ๆ
- ข้อความบน badge: เขียว/แดง ใช้ตัวอักษร `#ffffff`, เหลือง ใช้ `#1f2933` (อ่านง่าย)

**ลิงก์ approve-gate ต้องชี้ UAT เท่านั้น:** `https://bpuu-service-uat.kmutt.ac.th/approve-gate?...`
(ห้าม localhost / prod — ขึ้น PROD ค่อยแก้ทีเดียว)
